...
- **UUID Routing & Security (Critical):** Verify that all public API routes and payloads utilize UUIDs (or secure public tokens) to identify resources. Auto-incrementing numeric IDs (e.g., `id = 1`) must NEVER be exposed in URL routes or request bodies.
