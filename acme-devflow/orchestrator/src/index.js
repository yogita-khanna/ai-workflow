...
const username = args[2]; // e.g. alice, bob, charlie, delta, john

const availableCommands = [
  'init', 'propose', 'approve-spec', 'design', 'approve-design',
  'swagger', 'approve-swagger', 'implement', 'review', 'approve-review',
  'document', 'merge', 'archive', 'status'
];

if (!command || !availableCommands.includes(command)) {
  console.log('Usage: node index.js <command> <ticket-id> [username]');
...
switch (command) {
  ...
  case 'propose':
    ...
    break;
  case 'approve-spec':
    orchestrator.approveSpec(username);
    break;
  case 'design':
    orchestrator.design();
    break;
  case 'approve-design':
    orchestrator.approveDesign(username);
    break;
  case 'swagger':
    orchestrator.swagger();
    break;
  case 'approve-swagger':
    orchestrator.approveSwagger(username);
    break;
  case 'implement':
  ...
  case 'review':
    ...
    break;
  case 'approve-review':
    orchestrator.approveReview(username);
    break;
  case 'document':
    orchestrator.document();
    break;
  case 'merge':
    orchestrator.merge(username);
    break;
  case 'archive':
  ...
  case 'status':
    ...
    console.log(`Checkpoint 2b (Swagger): ${orchestrator.ticket.swaggerApproved ? '\x1b[32mApproved\x1b[0m' : '\x1b[31mPending\x1b[0m'}`);
    ...
    logs.forEach(log => {
      const userPart = log.user ? ` (${log.user})` : '';
      console.log(`[${log.timestamp.split('T')[1].slice(0,8)}] \x1b[1m${log.action}\x1b[0m${userPart} - ${log.details}`);
    });
    console.log();
    break;
}
