...
describe('UserProfileService', () => {
  ...
  describe('updateProfile', () => {
    it('should return updated profile when repository updates it', async () => {
      ...
      const result = await service.updateProfile('1', 'John', 'Doe', 'avatar.jpg');
      expect(result).toEqual(mockProfile);
      expect(mockRepository.updateProfile).toHaveBeenCalledWith('1', 'John', 'Doe', 'avatar.jpg');
    });

    it('should throw an error if profile to update is not found', async () => {
      mockRepository.updateProfile.mockResolvedValueOnce(null);

      await expect(service.updateProfile('1', 'John', 'Doe', 'avatar.jpg')).rejects.toThrow('User not found');
    });
  });
});
