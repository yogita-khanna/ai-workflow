import { SystemSettingsController, UpdateSettingsDto } from './system-settings.controller';
...
describe('SystemSettingsController', () => {
  ...
  describe('getSettings', () => {
    it('should return settings for authenticated user', async () => {
      ...
      const mockSettings = { user_id: 'uuid-1', theme: 'light', notifications_enabled: true } as any;
    ...
    it('should throw error if user not authenticated', async () => {
      ...
      await expect(controller.getSettings(mockRequest as any)).rejects.toThrow('Unauthorized');
    });
  });
  ...
  describe('updateSettings', () => {
    it('should update settings for authenticated user', async () => {
      const mockRequest = { user: { id: 'uuid-1' } };
      const dto: UpdateSettingsDto = { theme: 'dark', notifications_enabled: false };
      const mockSettings = { user_id: 'uuid-1', theme: 'dark', notifications_enabled: false } as any;
      ...
      expect(result).toEqual(mockSettings);
      expect(mockService.updateSettings).toHaveBeenCalledWith('uuid-1', 'dark', false);
    });
  });
});
